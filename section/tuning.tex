...
Then
\begin{equation}
	\tilde{H} \ket{a,b} = \sum_{c} \tilde{\kappa}_{ca}\ket{c,b} + \sum_{c} k_{cb}\ket{ac} + \tilde{V}_{ab} \ket{a,b}
\end{equation}
...
\begin{align}
    \ket{P,r} = \oneover{N_x} \sum_a e^{iPa} \ket{a,a+r}
\end{align}
Then applying the (interacting) Hamiltonian gives
\begin{align}
    \tilde{H} \ket{P,r}
    &=
            \sum_{r'} \tilde{\kappa}_{rr'} \left( e^{iP(r'-r)}+1\right)\ket{P,r'}
...
In \Figref{tune three spheres} we tune $C_{\vec{0}}$, $C_{(0,1)}$ and $C_{(1,1)}$ to the three lowest energies.
