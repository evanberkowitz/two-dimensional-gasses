...
\subsection{Hamiltonian, Continuum, Infinite-Volume, and Zero-Temperature Limits}
...
\begin{align}
    \tilde{a} &= \frac{2\pi a}{N_x \Delta x} = \frac{2\pi a}{L}
    &
    \tilde{\beta} &= \frac{\beta}{M \Delta x^2} = \frac{\beta}{ML^2} N_x^2
    \nonumber\\
    \tilde{\mu} &= \mu M \Delta x^2 = \mu ML^2 N_x^{-2}
    &
    \tilde{\vec{h}} &= \vec{h} M \Delta x^2 = \vec{h} ML^2 N_x^{-2}
    \\\nonumber
    \tilde{C}_0 &
...
To simplify the other limits of interest we trade $\mu$ and $\vec{h}$ for fugacities $z$ and $\vec{z}_h$,
\begin{align}
...
What's nice about $\log \tilde{z}$ and $\log \tilde{z}_h$ is that they take the same values no matter $L$ or $N_x$; they're intrinsic, dimensionless, and scale-independent, free from $N_x$ or $\Delta x$.

The \emph{infinite lattice limit} holds $\Delta x$ fixed and sends $L$ and $N_x$ to infinity with $L/N_x = \Delta x$ fixed, a sort of infinite volume limit; imagine a very large square crystal.
While $\tilde{a}$ goes to 0, $\tilde{C}_0$ remains fixed.
Dimensionless $\tilde{\beta}$, $\log \tilde{z}$, $\widevec{\log \tilde{z}_h}$, \emph{all} stay fixed: all that changes is $N_x$.

The \emph{continuum limit} holds $a$, $L$, $\beta$, $\mu$, and $\vec{h}$ fixed and sends $N_x \goesto \infty$ as $\Delta x\goesto 0$ so that $L=N_x \Delta x$ is fixed.
Since $\tilde{a}$ is fixed but $N_x$ increases, $\tilde{C}_0$ decreases.
The continuum limit holds $\tilde{a}$, $\log \tilde{z}$ and $\widevec{\log \tilde{z}_h}$ fixed while scaling $\tilde{\beta}$ with $N_x^2$ and decreasing $\tilde{C}_0$.

The \emph{zero temperature} and \emph{infinite volume} limits are tricky to disentangle.
