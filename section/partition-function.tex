...
\begin{align}
	N
	&\goesto
	\sum_x \Delta x^2 (n_x = \psi\adjoint_x \psi_x)
	=
	...
	\vec{S}
	&\goesto
	\half \sum_x \Delta x^2 (\vec{s}_x = \psi\adjoint_x \vec{\sigma} \psi_x)
	=
	\half \sum_a (\tilde{\psi}\adjoint_a ~ \vec{\sigma} ~ \tilde{\psi}_a = \tilde{s}_a)
	=
	\tilde{\vec{S}}
\end{align}
...
The computation of $\UU$ and related quantities requires iterated multiplication of ill-conditioned matrix exponentials.
\R{Bauer:2020} reviews techniques for numerically stabilizing their construction.
...
\begin{align}
	...
	\label{eq:partition function equality}
\end{align}
...
For a detailed discussion of this operator see \R{Wynen:2018ryx} (it is the `exponential $\alpha=0$' operator), where it is shown $\dd$ has an ergodicity problem when the lattice is bipartite (if $\vec{h}=0$).
...
\begin{align}
	...
	\label{eq:fermionic derivatives}
\end{align}
...
\begin{align}
    \frac{\tilde{\beta}}{\tilde{a}^2}
    = \frac{\beta}{(2\pi a)^2 M}
    = \frac{2\pi}{MT} \oneover{(2\pi)^3 a^2}
    &= \oneover{(2\pi)^3} \left(\frac{\deBroglie}{a}\right)^2
    &
    \deBroglie^2 &\equiv \frac{2\pi}{MT}
    \label{eq:aspect ratio}
\end{align}
fixed where \deBroglie is the thermal de Broglie wavelength.
Some experiments~\cite{PhysRevLett.114.230401} quote $\beta\binding$ where $\binding=-1/Ma^2$ is the binding energy \eqref{binding energy}, which differs only from the aspect ratio by $(2\pi)^2$.
