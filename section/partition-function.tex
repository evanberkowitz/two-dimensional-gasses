...
\begin{align}
	...
	\\
	&= \exp\left(\tilde{\beta}\tilde{\vec{h}}\cdot\vec{\sigma}\right) \begin{pmatrix} zU & 0 \\ 0 & zU \end{pmatrix}
	&
...
\begin{align}
	\det \dd 
	= \det(\one+\UU)
	&= \det\left(\one + e^{\tilde{\beta} \tilde{\vec{h}}\cdot\vec{\sigma}}(zU\otimes\one)\right)
	= \det\left(\one + O\inverse O e^{\tilde{\beta} \tilde{\vec{h}}\cdot\vec{\sigma}} (zU\otimes\one)\right)
	\nonumber\\
	&= \det\left(\one + O e^{\tilde{\beta} \tilde{\vec{h}}\cdot\vec{\sigma}} (zU\otimes\one)O\inverse \right)
	= \det\left(\one + O e^{\tilde{\beta} \tilde{\vec{h}}\cdot\vec{\sigma}} O\inverse (zU\otimes\one) \right)
	\nonumber\\
	&= \det\left(\one + e^{\tilde{\beta} \abs{\tilde{h}} \sigma_3 } (zU\otimes\one) \right)
	\\
	&= \det\left( 1 + e^{+\tilde{\beta}\abs{\tilde{h}}} zU\right)\det\left( 1 + e^{-\tilde{\beta}\abs{\tilde{h}}} zU\right)
...
\begin{align}
	...
	&= \delta_{tt'} \FF_{t'} \PP_a (\hat{h}\cdot\sigma).
\end{align}
