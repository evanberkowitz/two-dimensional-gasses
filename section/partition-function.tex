...
Note that we can separate $\FF_t = e^{A_{t}+\Delta\tilde{t}\tilde{\mu} + \Delta\tilde{t}\tilde{\vec{h}}\cdot\vec{\sigma}}$ into a space-dependent spin-independent part $e^{A_t}$ and a space-independent spin-dependent part $\exp( \Delta\tilde{t}(\tilde{\mu} \one + \tilde{\vec{h}} \cdot \vec{\sigma})$ (though the $\mu$ pieces is spin independent!).
...
Since $\BB$ only changes with the hopping parameter or with the lattice geometry, we focus on derivates of $\FF$.
\begin{align}
	\oneover{\Delta\tilde{t}} \frac{\partial}{\partial \tilde{\mu}_{t,a}} \FF_{t'} &= \delta_{tt'} \FF_{t'} \PP_a
\end{align}
where $\PP_a$ is a projector to site $a$.
This derivative is simple because $\tilde{\mu}$ appears inside the matrix exponential $\FF$ \eqref{FF} coupled to the (spin) identity.
