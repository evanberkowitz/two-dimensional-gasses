...
\end{align*}
This new HS transformation looks pretty ugly but it's really not that bad. The only element of the story that changes is the matrix element
\begin{align*}
...
\end{equation*}
I know two ways to compute these correlation functions: functional derivatives and operator insertions. The operator insertion method involves recomputing the trace  with Grassmann variables with the operator inserted; I have several operators worked out this way in my notes, but it's really laborious. Functional derivatives produce the exact same answer and are easier to compute (assuming the chemical potential in the $V$ rather than the $K$ term of the Trotterization), so we'll do that here.
