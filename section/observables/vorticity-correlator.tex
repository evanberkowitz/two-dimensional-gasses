...
\R{PhysRevLett.129.076403} provides an analysis of the BKT transition by studying the condensate fraction [their (2)] and the Cooper pair momentum distribution [their (3)].
...
We give an example calculation of \incode{docs/sanity-checks/free-theory-continuum-limit.ipynb}{the free-theory continuum extrapolation}, which shows a staggered behavior based on $N_x\mod{4}$ and \incode{docs/sanity-checks/free-theory-infinite-volume-limit.ipynb}{the free-theory infinite volume extrapolation} (at fixed lattice spacing) which shows that these $b_n/k_F^{6-n}$ are intensive.
