...
To write a custom observable is quite simple: you write a function that takes an ensemble and computes the observable you're interested in, and `decorate`_ it with :code:`@observable` or :code:`@callable_observable`.

For example, here is the actual implementation of :func:`~.average_field`

.. literalinclude:: ../../tdg/observable/field.py

See `the implementation`_ of :func:`~.number.n` for an example implementation of a :code:`@callable_observable`.
